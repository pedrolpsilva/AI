    ...
    detail(place: string) {
        return this.connection.query(`
            SELECT TOP 1
                ...
                mm.evento,
                IIF ( mm.evento = 0, 'Normal', b.eventDesc) AS tecnologia,
                cv.idCategoria AS categoria,
                cc.pinPadrao,
                m.nome AS motorista
            FROM movimento_vl02 mm
                JOIN cadastro_veiculo cv ON cv.serial = mm.serial
