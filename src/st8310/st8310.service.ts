...
export class St8310Service {
    ...
    monitoring(place: string) {
        return this.connection.query(`
            ...
            FROM movimento_suntech4g_frontend mm
    ...
    async command(date: string, commandName: string, place: string, idClient: number, idUser: number, command: string, serial: string, randomNumber: number) {
        await this.connection.query(`
            INSERT INTO
                semaforo_suntech4g (data, comandoNome, placa,idClienteLog,idUsuarioLog, comando ,serial , numeroComum)
        ...
        .then( async (res) => {
            if (res.rowsAffected > 0) {
                await this.connection.query(`
                    INSERT INTO
                        bkp_semaforo_suntech4g (data, comandoNome, placa,idClienteLog,idUsuarioLog, comando ,serial , numeroComum)
                        VALUES ('${date}','${commandName}','${place}', ${idClient}, ${idUser}, '${command}', ${serial}, '${randomNumber}')  
                `)
                .then((res2) => {
    ...
    reportCommand(places: string, dateI: string, dateF: string) {
        return this.connection.query(`
            SELECT
                ...
                mm.id,
                b.nome
            FROM bkp_semaforo_suntech4g mm
