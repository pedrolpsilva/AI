...
export class Vl03Service {
    ...
    monitoring(place: string) {
        return this.connection.query(`
            ...
            FROM movimento_vl03_frontend mm
                JOIN cadastro_veiculo cv ON cv.placa = mm.placa
                LEFT JOIN evento_vl03 e ON e.id = mm.evento
        ...
        if (profile == 3) {
            return this.connection.query(`
                ...
                FROM Veiculos v
                    ...
                    LEFT JOIN assoc_veiculos_usuarios asu2 ON CAST(asu2.id_veiculo AS VARCHAR(50)) = v.idVeiculo AND asu2.id_usuario = ${user}
    ...
    async command(date: string, commandName: string, place: string, idClient: number, idUser: number, command: string, serial: string, randomNumber: number) {
        await this.connection.query(`
            INSERT INTO
                semaforo_vl03 (data, comandoNome, placa,idClienteLog,idUsuarioLog, comando ,serial , numeroComum)
        ...
        .then( async (res) => {
            if (res.rowsAffected > 0) {
                await this.connection.query(`
                    INSERT INTO
                        bkp_semaforo_vl03 (data, comandoNome, placa,idClienteLog,idUsuarioLog, comando ,serial , numeroComum)
                        VALUES ('${date}','${commandName}','${place}', ${idClient}, ${idUser}, '${command}', ${serial}, '${randomNumber}')  
                `)
                .then((res2) => {
