...
@Controller('gv75')
export class Gv75Controller {
    constructor(private readonly service: Gv75Service, private readonly authService: AuthService) {}
    ...
    async command(@Body() req: commandDto) {
        return this.service.command(newDate(), req.commandName, req.place, req.idClient, req.idUser, req.command, Math.floor(Math.random() * 10000) + 1)
    }
